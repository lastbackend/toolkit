...
func (c *core) Write(ent zapcore.Entry, fs []zapcore.Field) error {
	...
	fmt.Println(c.cfg.Level.Enabled(ent.Level), c.cfg.Level, ent.Level)
...
func getScope(field zapcore.Field) *sentry.Scope {
	if field.Type == zapcore.SkipType {
		if scope, ok := field.Interface.(*sentry.Scope); ok && field.Key == sentryScopeKey {
			return scope
		}
	}
...
func filterFrames(frames []sentry.Frame) []sentry.Frame {
	...
	for i := range frames {
		if (strings.HasPrefix(frames[i].Function, "go.uber.org/zap")) &&
